...
import { CreateUserTenantMappingDto, UserTenantMappingResponseDto } from './user-tenant-map.dto';
...
export class UserTenantMapController {
  ...
  @Post()
  async createMapping(@Body() mappingData: CreateUserTenantMappingDto): Promise<UserTenantMappingResponseDto> {
  ...
  @Get(':tenantId/:userId')
  async getMapping(@Param('tenantId') tenantId: string, @Param('userId') userId: string): Promise<UserTenantMappingResponseDto | null> {
    return await this.userTenantMapService.getUserTenantMapping(tenantId, userId);
  }

  @Get('tenant/:tenantId')
  async getMappingsByTenant(@Param('tenantId') tenantId: string): Promise<UserTenantMappingResponseDto[]> {
    return await this.userTenantMapService.getMappingsByTenantId(tenantId);
  }

  @Get('user/:userId')
  async getMappingsByUser(@Param('userId') userId: string): Promise<UserTenantMappingResponseDto[]> {
    return await this.userTenantMapService.getMappingsByUserId(userId);
  }

  @Delete(':tenantId/:userId')
  async deactivateMapping(@Param('tenantId') tenantId: string, @Param('userId') userId: string): Promise<{ message: string }> {
