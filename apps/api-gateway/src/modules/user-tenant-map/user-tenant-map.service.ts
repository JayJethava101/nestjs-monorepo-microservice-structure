...
export class UserTenantMapService {
  ...
  async createUserTenantMapping(tenantId: string, userId: string): Promise<UserTenantMap> {
  ...
  async getUserTenantMapping(tenantId: string, userId: string): Promise<UserTenantMap | null> {
    return await this.userTenantMapRepository.findOne({
      where: { tenantId, userId, active: true }
    });
  }

  async deactivateUserTenantMapping(tenantId: string, userId: string): Promise<void> {
