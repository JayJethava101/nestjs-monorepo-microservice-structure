...
export class UserController {
  ...
  async findOne(data: { id: string }, metadata: Record<string, any>): Promise<any> {
    const { tenantId, dbName } = this.getTenantInfo(metadata);
    return this.userService.findOne(data.id, { tenantId, dbName });
  }

  @GrpcMethod('UserService', 'UpdateUser')
  async update(data: { id: string } & UpdateUserDto, metadata: Record<string, any>): Promise<any> {
    const { tenantId, dbName } = this.getTenantInfo(metadata);
  ...
  async remove(data: { id: string }, metadata: Record<string, any>): Promise<{ success: boolean }> {
    const { tenantId, dbName } = this.getTenantInfo(metadata);
  ...
  async findAllWithNameCount(data: { page?: number; limit?: number; search?: string; sort?: string; order?: 'ASC' | 'DESC' }, metadata: Record<string, any>): Promise<{ items: { name: string, characters: number, symbols: number }[], total: number }> {
    return {
      items: [{
        name: "jay",
        characters:3,
